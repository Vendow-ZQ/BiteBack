...
卡片和 Sheet 的展示文案必须集中放在 `src/mocks/copy.json`，包括：

- 卡片标题、副标题、AI 标签、底部提示。
- 四个卡片页的区块标题、解释句、按钮文案。
- “马上开吃” Sheet 的标题、路线行动、评论证明、团购弱提示。
- “不感兴趣”反馈项、Toast、解释 Sheet 文案。

组件只允许从 `copy.json` 读取文案并用 `formatCopy` 注入变量，不要在组件内硬编码可展示给用户的中文句子。后续产品同学改表达时，只改 JSON。

### 4.0.2 素材占位与替换

BiteBack 卡片里的食物图、门店图、路线图、原收藏封面、头像，在没有真实素材前一律使用 `placeholderColor` 色块，不要引用旧 demo 截图、真实抖音参考截图或错误路径。

后续替换真实素材时：

1. 把图片放进 `public/assets/food`、`shops`、`maps`、`covers`、`avatars` 对应目录。
2. 在 `src/mocks/assets.ts` 给对应素材 ID 增加 `path`。
3. 业务数据仍然只引用 `assetId`。
4. React 组件不改路径、不写文件名、不接地图 API key。
...
- BiteBack 业务图片位不得引用真实抖音参考截图或旧 demo 截图；没有素材就保持色块。
- 卡片、Sheet、按钮、反馈、Toast 文案集中在 `src/mocks/copy.json`，组件内不硬编码展示文案。
